...
Run with: conda run -n DT_trajopt python test_unified_model.py
"""
...
params = VehicleParams(
    ...
    brake_f_frac=0.6,
    brake_r_frac=0.4,
    p_eng_max_w=172000.0,
    max_delta_deg=27.0,
    max_delta_dot_degps=20.0,
    max_fx_kn=10.0,
    min_fx_kn=-15.0,
    max_fx_dot_knps=10.0,
    min_fx_dot_knps=-10.0,
    tau_long_weight_transfer_s=0.1,
    tau_lat_weight_transfer_s=0.1,
    roll_rate_radpmps2=0.05,
    gamma_none=0.64,
)

model = SingleTrackModel(params, f_tire, r_tire, enable_weight_transfer=True)
print(f"  Created: {model}")
print(f"  Wheelbase: {params.l_m:.2f} m")
print(f"  Front weight: {params.wf_n:.0f} N")
print(f"  Rear weight: {params.wr_n:.0f} N")
...
f_grade_expected = -params.m_kg * 9.81 * np.sin(theta_uphill)
a_grade_expected = f_grade_expected / params.m_kg
...
f_bank_expected = -params.m_kg * 9.81 * np.sin(phi_bank)
a_bank_expected = f_bank_expected / params.m_kg
...
print(f"  (Positive = load transferring to front)")
