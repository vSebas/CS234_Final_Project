...
- TRACK2: D-shaped + serpentine on the straight
- TRACK3_300m: Serpentine (continuous curves, no straights)
- TRACK4_315m: Technical (procedural, sharp corners)
- TRACK5_330m: Imported custom track (maps/newtrack5.mat)
"""
...
def create_oval_track_with_obstacles(
    ...
    # Generate obstacles with the shared routine so all tracks use the same method.
    sio.savemat(output_filename, data)
    return add_obstacles_to_map(output_filename, output_filename)
...
def add_obstacles_to_map(
    ...
    if num_obstacles is None:
        num_obstacles = min(4, max(1, int(round(length_m / 60.0))))
...
def create_track_from_waypoints(
    output_filename: str,
    ...
    """
    Create a track from manually specified waypoints.

    Parameters:
        waypoints: List of (x, y) control points forming a closed loop.
        total_length: Target track length in meters (None = use natural size from waypoints)
    ...
    tck, u = interpolate.splprep([pts_closed[:, 0], pts_closed[:, 1]], s=smoothing, per=True)

    dense = 2000
    t_dense = np.linspace(0, 1, dense, endpoint=False)
    ...
    # Only scale if total_length is specified, otherwise use natural length
    ...
    ds = s_m[1] - s_m[0]
    dE = np.gradient(posE_m)
    dN = np.gradient(posN_m)
    psi_rad = np.unwrap(np.arctan2(dN, dE))
    psi_s = np.gradient(psi_rad, ds)
    psi_ss = np.gradient(psi_s, ds)
    ...
    # Save original waypoints for re-editing
    ...
    data = {
        ...
        "inner_bounds_m": inner,
        "outer_bounds_m": outer,
        "waypoints_xy": wp_array,
    }
    sio.savemat(output_filename, data)
    print(f"Waypoint track: length={computed_length:.1f}m, width={track_width:.1f}m, "
          f"min_R={min_radius:.1f}m, {len(waypoints)} waypoints")
    return data
...
def _build_map_from_inner_offset(
    ...
    if target_width_m is not None and target_width_m > 0.0:
        hw = 0.5 * float(target_width_m)
        inner, outer = _compute_bounds_from_centerline(
            posE_m, posN_m, psi_rad, np.full(num_samples, hw), np.full(num_samples, hw)
        )
    inner_xy = inner[:, :2] if inner.ndim == 2 and inner.shape[1] >= 2 else inner
    outer_xy = outer[:, :2] if outer.ndim == 2 and outer.shape[1] >= 2 else outer
    width = np.linalg.norm(outer_xy - inner_xy, axis=1)
...
def create_imported_track(
    source_filename: str,
    output_filename: str,
    label: str,
) -> dict:
    data = sio.loadmat(source_filename, squeeze_me=True)
    sio.savemat(output_filename, data)
    length_m = float(np.atleast_1d(data["length_m"]).item())
    width = np.atleast_1d(data["track_width_m"]).astype(float)
    print(
        f"{label}: length={length_m:.2f} m, width_mean={float(np.mean(width)):.2f} m, "
        f"width_min={float(np.min(width)):.2f} m, width_max={float(np.max(width)):.2f} m"
    )
    return data
...
def main():
    ...
    # Use script directory as maps directory
    maps = Path(__file__).parent
    ...
    if args.preset in ("technical", "all"):
        out = maps / "TRACK4_315m.mat"
        create_technical_track(
            str(out),
            seed=42,
            target_length_m=315.0,
            track_width=6.0,
        )
        print(f"Saved {out}")

    if args.preset in ("imported", "all"):
        out = maps / "TRACK5_330m.mat"
        source = maps / "newtrack5.mat"
        if not source.exists():
            raise FileNotFoundError(f"Missing source track: {source}")
        create_imported_track(str(source), str(out), label="Imported track5")
        print(f"Saved {out}")
