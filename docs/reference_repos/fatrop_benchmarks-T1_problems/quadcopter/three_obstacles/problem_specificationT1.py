...
class QuadCopterThreeObs(QuadcopterDynamics):
    def __init__(self):
        ...
        self.ocp.set_initial(self.T, 10.0)
        # parameters
        ...
        self.p_obs1 = np.array(0.2*p0+0.8*pf)
        self.or_pobs1 = "z"
        self.r_obs1 = 2.0 

        self.p_obs2 = np.array(0.5*p0 + 0.5*pf)  + np.array([[0., 0., 1.]]).T
        self.or_pobs2 = "y"
        self.r_obs2 = 2.0 

        self.p_obs3 = np.array(0.8*p0 + 0.2*pf) +np.array([[-1., 0., 0.]]).T
        self.or_pobs3 = "z"
        self.r_obs3 = 2.0 
        ...
        self.softconstr2 = sqrt_special(distsq(self.p[[0,2]], self.p_obs2[[0,2]]))-(self.r_obs2+r_drone)
        self.softconstr3 = sqrt_special(distsq(self.p[:2], self.p_obs3[:2]))-(self.r_obs3+r_drone)
...
if __name__ == "__main__":
    ...
    SolveIpopt(QuadCopterThreeObs(), 100, jit =False)