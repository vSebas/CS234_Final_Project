...
class QuadcopterDynamics:
    def __init__(self, ocp: rockit.Ocp):
        ...
        ocp.set_der(self.p, self.v)
        ocp.set_der(self.v, a)

        ocp.subject_to(-np.pi/2 <= (self.phi <= np.pi/2), include_last=False, include_first=False)
        ocp.subject_to(-np.pi/2 <= (self.theta <= np.pi/2), include_last=False, include_first=False)

        ocp.subject_to(cs.cos(self.theta)*cs.cos(self.phi) >= cs.cos(tiltmax), include_last=False, include_first=True)
