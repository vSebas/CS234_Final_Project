...
class Capsule:
    ...
    def point_distance_constraint_sqrt(self, point, min_dist):
        ## the value of this expression should be larger than zero
        ## point coordinates in local frame
        point_local = (self.T_capsule_world @ cs.vertcat(point, np.array([1.0])))[:3]
        x_l = point_local[0]
        y_l = point_local[1]
        abs_z_l = cs.fabs(point_local[2])
        ## distance to cylinder vs distance to ball
        return cs.if_else(abs_z_l<self.length/2, cs.sqrt(x_l*x_l+ y_l*y_l) - min_dist- self.radius, cs.sqrt(x_l*x_l+y_l*y_l+(abs_z_l-self.length/2)**2)- (min_dist+ self.radius))
    def point_distance_constraint_sphere(self, point, min_dist):
        ## the value of this expression should be larger than zero
        ## point coordinates in local frame
    ...
    def point_distance_constraint_old(self, point, min_dist):
        ## the value of this expression should be larger than or equal to zero
        ## point coordinates in local frame
        origin_caps = self.T_world_capsule[:3,3]
        delta = point - origin_caps
        ...
        projected_length_abs = cs.fabs(projected_length)
