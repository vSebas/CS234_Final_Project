...
class CartPendulumDynamics:
    def __init__(self, ocp: rockit.Ocp):
        ...
        ocp.set_der(self.x, self.dx * self.T)
        ocp.set_der(self.dx, ddx_ * self.T)
        ocp.set_der(self.theta, self.omega*self.T)
        ocp.set_der(self.omega, alpha_ * self.T)
        ocp.subject_to(-max_f < (self.Fex < max_f), include_last=False)
        ocp.subject_to(-max_x < (self.x < max_x), include_last=False)
        ocp.subject_to(-max_v < (self.dx < max_v), include_last=False)
