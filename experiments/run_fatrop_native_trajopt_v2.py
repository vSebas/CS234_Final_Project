...
def solve_fatrop_native(
    ...
    *,
    ...
    opti = ca.Opti()
    # Variables must be created stage-interleaved (x_0, u_0, x_1, u_1, ..., x_N)
    # so that FATROP auto structure detection can assign them to stages correctly.
    x_vars = []
    u_vars = []
    for k in range(N):
        x_vars.append(opti.variable(nx))
        u_vars.append(opti.variable(nu))
    x_vars.append(opti.variable(nx))   # terminal state x_N
    ...
    for k in range(N):
        ...
        cons = []
        # Initial-time boundary condition inside stage 0 so FATROP auto structure
        # detection treats it as a stage-0 path constraint rather than a pre-stage
        # constraint (which causes all downstream dynamics to appear cross-stage).
        if k == 0:
